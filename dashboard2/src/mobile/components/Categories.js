...
const Categories = ({ User, changeCategoryHandler }) => {
    ...
    return (
        <Container sx={{ height: 130, mt: 1, backgroundColor: 'white', overflowY: 'hidden', overflowX: 'scroll', boxShadow: 1 }}>
            <Typography sx={{ pt: 0, color: '#6f86d6', fontWeight: 800 }} color="textPrimary" variant="h5">
                Explore Categories
            </Typography>
            <Box sx={{ display: 'flex', overflowX: 'scroll', m: 0 }}>
                <Box sx={{ ml: 1, mt: 0, display: 'flex', flexDirection: 'column' }}>
                    ...
                    >
                        ...
                        <Typography sx={{ pt: 1, color: '#6f86d6', fontWeight: 800 }} color="textPrimary" variant="h6">
                            View All
                        </Typography>
                    </Button>
                </Box>
                {categories?.map((cat) => (
                    <Box key={cat._id} sx={{ ml: 1, mt: 0, display: 'flex', flexDirection: 'column' }}>
                        <Button
                            className={classes.button}
