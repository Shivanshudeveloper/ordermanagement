...
const Menu = () => {
    ...
    const handleClose = (men) => {
        ...
        if (men !== null) {
            ...
            const addMenuFunction = async () => {
                try {
                    const rawResponse = await fetch(`${API_SERVICE}/api/v1/main/menu/addmenu`, {
                        method: 'POST',
                        headers: {
    ...
    const updateMenu = (menu) => {
        ...
        const updateMenuList = async () => {
            try {
                const rawResponse = await fetch(`${API_SERVICE}/api/v1/main/menu/updatemenu/${menu._id}`, {
                    method: 'PATCH',
                    headers: {
                        Accept: 'application/json',
                        'Content-Type': 'application/json'
    ...
    const handleDeleteMenu = async (id) => {
        const newMenu = menuList.filter((m) => m._id !== id);
        const newFilteredList = filteredMenuList.filter((m) => m._id !== id);

        try {
            const rawResponse = await fetch(`${API_SERVICE}/api/v1/main/menu/removemenu/${id}`, {
                method: 'delete'
            });
    ...
    const searchHandler = (value) => {
        const filteredCurrent = menuList.filter((obj) => obj.item.toLowerCase().includes(value.toLowerCase()));
