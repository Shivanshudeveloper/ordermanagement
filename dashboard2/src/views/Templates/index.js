...
import { Box, FormControl, InputLabel, Container, Grid, Select, MenuItem, Typography, CircularProgress, Button } from '@mui/material';
...
const Templates = () => {
    ...
    const setlayoutfunction = async (grid) => {
        try {
            const rawres = await fetch(`${API_SERVICE}/api/v1/main/user/setlayout`, {
                method: 'PATCH',
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ grid, email: User.email })
            });
